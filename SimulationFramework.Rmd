...
SIM.Comp <- function(tsteps = 3e3, d0 = 0.2, b0 = 0.8,
  ...
  ## object creation
  ### data frame that stores grid coordinates, GridX holds the left-most starting-point of each gridcell
  grids_df <- data.frame(
    GridID = 1:n_Grid,
    GridX = seq(from = Env_range[1], to = Env_range[2], length = n_Grid+1)[-(n_Grid+1)]
  )
  ### data frame to hold population level data
  GridsID_vec <- sapply(ID_df$Location, FUN = function(x){tail(which(x >= grids_df$GridX), 1)}) # identify gridcell for each individual
  Pop_ls <- list(table(GridsID_vec, ID_df$Species)) # store populations of each species binned by gridcell as a list element 
  Pop_df <- data.frame(N = nrow(ID_df),
                       t = NA)
  ### progress bar
  pb <- txtProgressBar(max = (tsteps-1), style = 3)
  ...
  GridsID_vec <- sapply(ID_df$Location, FUN = function(x){tail(which(x >= grids_df$GridX), 1)}) # identify gridcell for each individual
  Pop_ls <- c(Pop_ls, # store populations of each species binned by gridcell as a
...
GridPop_ls <- lapply(names(SIM_Grid$GriddedPopulations), FUN = function(x){
  ...
  DiffPerc <- (SIM_Grid$GriddedPopulations[[x]][,1]-SIM_Grid$GriddedPopulations[[x]][,2])/
      (SIM_Grid$GriddedPopulations[[x]][,1]+SIM_Grid$GriddedPopulations[[x]][,2])
...
TraitPlot <- ggplot(SIM_2SpecDiffTr$Individuals,
       aes(x = Location, y = Trait, pch = Species, col = Species)) +
  geom_point(size = 3) + 
  theme_bw()
